...
class MainActivity : SimpleActivity() {
    ...
    fun refreshMenuItems() {
        ...
        binding.mainMenu.getToolbar().menu.apply {
            ...
            findItem(R.id.refresh_current).isVisible = true
            findItem(R.id.go_to_downloads).isVisible = currentFragment is ItemsFragment
            findItem(R.id.copy_current_path).isVisible = currentFragment is ItemsFragment
        ...
        binding.mainMenu.apply {
            ...
            getToolbar().setOnMenuItemClickListener { menuItem ->
                ...
                when (menuItem.itemId) {
                    ...
                    R.id.refresh_current -> refreshCurrentTab()
                    R.id.go_to_downloads -> goToDownloadsFolder()
                    R.id.copy_current_path -> copyCurrentPath()
    ...
    private fun goToDownloadsFolder() {
        val downloadsPath = getDownloadsPath() ?: return
        openPath(downloadsPath, forceRefresh = true)
    }

    private fun copyCurrentPath() {
        val path = (getCurrentFragment() as? ItemsFragment)?.currentPath ?: return
        copyToClipboard(path)
        toast(R.string.path_copied)
    }
    ...
    private fun getDownloadsPath(): String? {
        ...
        if (!publicDownloadsPath.isNullOrEmpty() && File(publicDownloadsPath).exists()) {
            return publicDownloadsPath
        }

        val fallbackPath = "$internalStoragePath/Download"
        return if (File(fallbackPath).exists()) fallbackPath else null
    }
